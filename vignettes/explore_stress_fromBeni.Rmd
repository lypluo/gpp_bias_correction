...
f_hardening <- function(temp, param){
  ...
  xx<- temp
  xx <- (-1)*param["b"] * (param["c"]*xx - param["a"])
  yy <- 1 / (1 + exp(xx))
  return(yy)  
}
ggplot() +
  geom_function(fun = f_hardening, args = list(param = c("a" = -10, "b" = 0.15,"c"=0.5)),col="black") +
  geom_function(fun = f_hardening, args = list(param = c("a" = -10, "b" = 0.15,"c"=0.7)),col="tomato") +
  geom_function(fun = f_hardening, args = list(param = c("a" = -10, "b" = 0.15,"c"=1)),col="orange") +
  geom_function(fun = f_hardening, args = list(param = c("a" = -10, "b" = 0.15,"c"=2)),col="red") +
  geom_function(fun = f_hardening, args = list(param = c("a" = -10, "b" = 0.15,"c"=5)),col="blue") +
  xlim(-50, 30)
```
...
f_dehardening <- function(temp, param){
  ...
  # xx <- param["b"] * (xx - param["a"])
  xx<-temp
  xx<-(-1)*param["b"] * (xx - param["a"])
  yy <- 1 / (1 + exp(xx))
  return(yy)  
}
ggplot() +
  geom_function(fun = f_dehardening, args = list(param = c("a" = 10, "b" = 0.1)),col="black") +
  geom_function(fun = f_dehardening, args = list(param = c("a" = 10, "b" = 0.2)),col="red") +
  geom_function(fun = f_dehardening, args = list(param = c("a" = 10, "b" = 1)),col="orange") +
  xlim(-10, 200)
```
