...
par_new4<-c("a" = 0, "b" = 0.5, "c" = 50, "d" = 0.1)
...
#
lower_new4=c(-50,0,0,0)
upper_new4=c(50,20,500,20)
...
# save(optim_par_old,file = paste0("../data/parameters/addNL_test/","optim_par_oldmod_run100_beni_addNL_DBF.rds"))
# 
...
# toc()
# #save the optimized data
# save(optim_par_new1,file = paste0("../data/parameters/addNL_test/","optim_par_newmod1_run100_beni_addNL_DBF.rds"))
# 
...
# toc()
# #save the optimized data
# save(optim_par_new2,file = paste0("../data/parameters/addNL_test/","optim_par_newmod2_run100_beni_addNL_DBF.rds"))
# 
...
# toc()
# #save the optimized data
# save(optim_par_new3,file = paste0("../data/parameters/addNL_test/","optim_par_newmod3_run100_beni_addNL_DBF.rds"))

##5) for new model strcture--remove the parameter e (model_hardening_new4)
tic("start to parameterize")
optim_par_new4 <- GenSA::GenSA(
  par = par_new4,
  fn = cost_new4,
  data = df_sel,
  lower = lower_new4,
...
toc()
#save the optimized data
save(optim_par_new4,file = paste0("../data/parameters/addNL_test/","optim_par_newmod4_run100_beni_addNL_DBF.rds"))
...
```{r message=FALSE,warning=FALSE}
load(paste0("../data/parameters/addNL_test/","optim_par_oldmod_run100_beni_addNL_DBF.rds"))
load(paste0("../data/parameters/addNL_test/","optim_par_newmod1_run100_beni_addNL_DBF.rds"))
load(paste0("../data/parameters/addNL_test/","optim_par_newmod2_run100_beni_addNL_DBF.rds"))
load(paste0("../data/parameters/addNL_test/","optim_par_newmod3_run100_beni_addNL_DBF.rds"))
load(paste0("../data/parameters/addNL_test/","optim_par_newmod4_run100_beni_addNL_DBF.rds"))
...
##stress factors for differenet model structure
  ...
  do({
    ...
    scaling_factor_new4 <- model_hardening_new4(.,optim_par_new4)
    data.frame(
      sitename = .$sitename,
      date = .$date,
      ...
      scaling_factor_optim_newmod4 = scaling_factor_new4
    )
  })
...
df_merge_new<-left_join(df_final,df_old,by = c("sitename", "date", "year")) %>%
  mutate(gpp_obs_recent=gpp,
         ...
         gpp_mod_recent_optim_newmod4=gpp_mod*scaling_factor_optim_newmod4,
         gpp=NULL,
         gpp_obs=NULL,
         gpp_mod=NULL)
...
# for (i in 1:length(PFTs)){
  ...
  recent_model_optim_newmod4_evas<-modeval(df_each$gpp_mod_recent_optim_newmod4,df_each$gpp_obs_recent,stat=c("N","pearson","MAE","RMSE","R2","slope","intercept","EF"))
  #
  stats_evals<-rbind(ori_model_evas,recent_model_evas,recent_model_optim_oldmod_evas,
                     ...
                     recent_model_optim_newmod3_evas,recent_model_optim_newmod4_evas)
  # stats_evals
  #merge different sites
...
for(i in 1:length(PFTs)){
  ...
  select(sitename,date,classid,gpp_obs_recent,gpp_mod_FULL_ori,gpp_mod_recent_ori,
         ...
         gpp_mod_recent_optim_newmod2,gpp_mod_recent_optim_newmod3,gpp_mod_recent_optim_newmod4) %>%
  mutate(gpp_obs=gpp_obs_recent,
         ...
         gpp_mod_recent_optim_newmod4=gpp_mod_recent_optim_newmod4
         ) %>%
...
gpp_modobs_comp_recent_optim_newmod4<-df_modobs_each %>%
  analyse_modobs2("gpp_mod_recent_optim_newmod4", "gpp_obs", type = "heat")
...
gpp_modobs_comp_recent_optim_newmod3$gg<-gpp_modobs_comp_recent_optim_newmod3$gg+
  annotate(geom="text",x=15,y=0,label=PFTs[i])
gpp_modobs_comp_recent_optim_newmod4$gg<-gpp_modobs_comp_recent_optim_newmod4$gg+
  annotate(geom="text",x=15,y=0,label=PFTs[i])

#merge two plots
evaulation_merge_plot<-plot_grid(gpp_modobs_comp_old_ori$gg,gpp_modobs_comp_recent_ori$gg,gpp_modobs_comp_recent_optim_oldmod$gg,gpp_modobs_comp_recent_optim_newmod1$gg,gpp_modobs_comp_recent_optim_newmod2$gg,gpp_modobs_comp_recent_optim_newmod3$gg,gpp_modobs_comp_recent_optim_newmod4$gg,
    widths=15,heights=10,
labels = "auto",ncol =3,nrow = 3,label_size = 12,align = "hv")
...
#plotting:
  ...
  summarise(obs = mean(gpp_obs, na.rm = TRUE),
            ...
            mod_recent_optim_newmod4=mean(gpp_mod_recent_optim_newmod4,na.rm = TRUE)) %>%
  pivot_longer(c(obs,mod_old_ori,mod_recent_ori,mod_recent_optim_oldmod,
            mod_recent_optim_newmod1,mod_recent_optim_newmod2,mod_recent_optim_newmod3,mod_recent_optim_newmod4), names_to = "Source", values_to = "gpp") %>%
  ggplot(aes(doy, gpp, color = Source)) +
  geom_line() +
  scale_color_manual(values = c("mod_old_ori" = "green","mod_recent_ori"="steelblue2","mod_recent_optim_oldmod" = "blue","mod_recent_optim_newmod1" = "tomato","mod_recent_optim_newmod2" = "red3","mod_recent_optim_newmod3" = "orange","mod_recent_optim_newmod4" = "purple3","obs" = "black"),
  labels = c("Old P-model","Recent Ori P-model", "Recent Optim old P-model",
             "Recent Optim new1 P-model","Recent Optim new2 P-model",
             "Recent Optim new3 P-model","Recent Optim new4 P-model","Obs.")) +
...
df_modobs %>%
  ...
  pivot_longer(c(obs,mod_old_ori,mod_recent_ori,mod_recent_optim_oldmod,
            mod_recent_optim_newmod1,mod_recent_optim_newmod2,
            mod_recent_optim_newmod3,mod_recent_optim_newmod4), names_to = "Source", values_to = "gpp") %>%
  ggplot(aes(doy, gpp, color = Source)) +
  geom_line() +
  scale_color_manual(values = c("mod_old_ori" = "green","mod_recent_ori"="steelblue2","mod_recent_optim_oldmod" = "blue","mod_recent_optim_newmod1" = "tomato","mod_recent_optim_newmod2" = "red3","mod_recent_optim_newmod3" = "orange","mod_recent_optim_newmod4" = "purple3","obs" = "black"),
  labels = c("Old P-model","Recent Ori P-model", "Recent Optim old P-model",
             "Recent Optim new1 P-model","Recent Optim new2 P-model","Recent Optim new3 P-model","Recent Optim new4 P-model","Obs.")) +
  labs(y = expression( paste("GPP (g C m"^-2, " d"^-1, ")" ) ),
       x = "Day of year") +
  annotate(geom="text",x=200,y=2,label="")+
  facet_wrap(~sitename)+
