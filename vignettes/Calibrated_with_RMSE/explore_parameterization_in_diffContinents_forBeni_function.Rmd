...
-for different continents: NorthAmerica, Europe, EastAsia(only two sites in Japan)
...
### optimize for different continets
...
# save(par_continents,file = paste0("../data/parameters/","optim_par_run100_beni_continents.rds"))
```
...
```{r message=FALSE,warning=FALSE}
load(paste0("../data/parameters/","optim_par_run100_beni_continents.rds"))
...
for (i in 1:length(continents)) {
  ...
  do({
    scaling_factor <- model_hardening(.,par_continents[[i]])
    data.frame(
      sitename = .$sitename,
      date = .$date,
...
evaulation_merge_plot<-plot_grid(gpp_modobs_comp1$gg,
    ...
    widths=20,heights=5,
labels = "auto",ncol =3,nrow = 1,label_size = 6,align = "hv")
...
```{r echo=FALSE,warning=FALSE,message=FALSE}
#Seasonal course for each sites in different continent:
# - for MF
df_modobs %>%
