...
# save(par_PFTs,file = paste0("../data/parameters/","optim_par_run100_koen_PFTs.rds"))
```
...
```{r message=FALSE,warning=FALSE}
load(paste0("../data/parameters/","optim_par_run100_koen_PFTs.rds"))
...
for (i in 1:length(PFTs)) {
  ...
  do({
    scaling_factor <- frost_hardiness(.$tmin,par_PFTs[[i]])
    data.frame(
      sitename = .$sitename,
      date = .$date,
