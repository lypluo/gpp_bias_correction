...
##also adding the function for f_L(function for the light)
##refer Mekela et al., 2004 and Mekela et al., 2008
##also add a scaler to improve the GPP in the mid-season
#################################################
#
f_Ts_rev_fL<-function(df,
     par=c("tau"=5,"X0"=-10,"Smax"=5,"lamba"=0.5,"k"=1),
     plot=FALSE){
  ...
  for (idx in seq(nrow(df))){
    ...
    f_Ts<-min(S/par["Smax"],1)
    #-----------
    #4):f_L:to add modifying factor for light
    #-----------
    f_L<-1/c(1+par["lamba"]*df$ppfd[idx])
    f_stress_temp<-f_Ts*f_L
    #---------
    #(5):stress factor multiplied by a scalar to
    ## allow for higher mid-season GPP after down-scaling early season GPP
    ## set different scaler at different period
    #between 6-22(summer solstice) and 9-22(autumn equinox):k1-->no T limitation
    #other period:k0
    #---------
    if(df$doy[idx]>172 & df$doy[idx]<266){
