...
#######################################################
#set two scaling factors(e and f) and normlized the data using Quantile95/90
#----------
library(tidyverse)
library(GenSA)

##working here:March,07
#-------------------------
...
cost <- function(
  ...
  #calculating the adjusted MSE for Clim.-PFTs:
  tt<-df %>%
    group_by(sitename)%>%
    summarise(mse=mean(c(gpp - gpp_mod * scaling_factor)^2),
              N=length(date)) %>%
    mutate(N_perc=N/sum(N))
  #
  mse=sum(tt$N_perc*tt$mse)
  #rmse
  # rmse <- sqrt(
  #   sum(
...
#----------------------
#only focusing on "Cfa-DBF" in this study:
#----------------------
Clim.PFTs<-"Cfa-DBF"
...
save(par_Clim_PFTs,file = paste0(base.path,"data/parameters_MSE_add_baseGDD/test/","optim_par_run5000_beni_Cfa_DBF_rev_2pars_norm.rds"))

#--------------------------------------------------------------
#(5) compare the gpp_obs, ori modelled gpp, and gpp modelled using optimated parameters
#--------------------------------------------------------------
load(paste0(base.path,"data/parameters_MSE_add_baseGDD/test/","optim_par_run5000_beni_Cfa_DBF_rev_2pars_norm.rds"))
print(par_Clim_PFTs)
...
unique(df_final_new[df_final_new$Clim_PFTs=="Cfa-DBF",]$sitename)


### make evaluation plots-->for Cfb-DBF
