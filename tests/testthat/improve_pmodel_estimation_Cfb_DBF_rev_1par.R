...
#only focusing on "Cfb-DBF" in this study:
Clim.PFTs<-"Cfb-DBF"
...
# save(par_Clim_PFTs,file = paste0(base.path,"data/parameters_MSE_add_baseGDD/test/","optim_par_run5000_beni_Cfb_DBF_rev.rds"))

#--------------------------------------------------------------
#(4) compare the gpp_obs, ori modelled gpp, and gpp modelled using optimated parameters
#--------------------------------------------------------------
load(paste0(base.path,"data/parameters_MSE_add_baseGDD/test/","optim_par_run5000_beni_Cfb_DBF_rev.rds"))
...
for (i in 1:length(Clim.PFTs)) {
  ...
  scaling_factors <- df_sel %>%
    ...
    do({
      scaling_factor <- model_hardening_1par(.,par_Clim_PFTs[[i]])
      data.frame(
        sitename = .$sitename,
        date = .$date,
...
unique(df_merge_new[df_merge_new$Clim_PFTs=="Cfb-DBF",]$sitename)
