...
#update in 2022-02-28-->using the parameter from different PFT and Clim. types:
load(paste0(base.path,"data/parameters_MSE_add_baseGDD/test/","optim_par_run5000_beni_Clim_andPFTs.rds"))
##merge the variable "Clim" and "PFT"
df_merge$Clim_PFTs<-paste0(df_merge$koeppen_code,"-",df_merge$classid)

##
Clim.PFTs<-names(par_Clim_PFTs)
#a.get the stress factor(calibration factor) for each Clim-PFT
df_final<-c()
for (i in 1:length(par_Clim_PFTs)) {
  df_sel<-df_merge %>%
  ...
  scaling_factors <- df_sel %>%
    ...
    do({
      scaling_factor <- model_hardening(.,par_Clim_PFTs[[i]])
      data.frame(
        sitename = .$sitename,
        date = .$date,
...
df_merge_new %>%
  ...
  summarise(gpp_obs=mean(gpp_obs_recent,na.rm=T),
            ...
            gpp_mod_new_optim=mean(gpp_mod_recent_optim,na.rm=T)
            )%>%
  pivot_longer(c(gpp_obs,gpp_mod_old,gpp_mod_new,gpp_mod_new_optim),
...
### make evaluation plots-->still evaluate the performance by according to different PFTs:
