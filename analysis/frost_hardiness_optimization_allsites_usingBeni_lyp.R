...
#refer the meaning of the parameters in logistic function:
...
scaling_factors <- df %>%
  ...
  do({
    scaling_factor <- model_hardening(.,par)
    data.frame(
      sitename = .$sitename,
      date = .$date,
...
scaling_factors <- df %>%
  group_by(sitename, year) %>%
  do({
    scaling_factor <- model_hardening(.,optim_par)
    data.frame(
      sitename = .$sitename,
      date = .$date,
...
ggsave("./fig/meanseasonalcycle_by_climate_cal_fromBeni.pdf", width = 9, height = 6)
...
ggsave("./fig/meanseasonalcycle_by_site_cal_fromBeni.pdf", width = 15, height = 30)
